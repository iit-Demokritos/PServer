 ...
 */
public class Cgetftrgroupfeatures implements pserver.pservlets.PService {
    ...
    public int service(VectorMap parameters, StringBuffer response, DBAccess dbAccess) {
        ...
        PSparameters.add("com", "getftrgroupfeatures");
       
        PSparameters.add("name", 
                parameters.getVal(parameters.indexOfKey("name", 0)));

        if (parameters.qpIndexOfKeyNoCase("ftrpattern") != -1) {
            PSparameters.add("ftrpattern", 
                    parameters.getVal(parameters.indexOfKey("ftrpattern", 0)));
        }
