 ...
 */
public class CfeatureAddComm implements pserver.pservlets.PService {
    ...
    public int service(VectorMap parameters, StringBuffer response, DBAccess dbAccess) {
        ...
        PSparameters.add("com", "addftrgrp");
        PSparameters.add("name", parameters.getVal(parameters.indexOfKey("communityname", 0)));
        PSparameters.add("ftrs", parameters.getVal(parameters.indexOfKey("features", 0)));

        if (parameters.qpIndexOfKeyNoCase("users") != -1) {
            PSparameters.add("usrs", parameters.getVal(parameters.indexOfKey("users", 0)));
        }
